...
function SubmissionDataViewer({ rows, title, defaultCollapsed = false }: { rows: MergedRow[]; title: string; defaultCollapsed?: boolean }) {
  ...
  const renderRow = (row: MergedRow) => {
    ...
    return (
      <div key={row.field} className="flex items-center justify-between h-9 border-b border-zinc-100 last:border-0 overflow-hidden">
        <span className="text-sm font-bold text-zinc-500 truncate pr-4 w-1/3 shrink-0" title={formatFieldName(row.field)}>
        ...
        <div className="flex-1 min-w-0 text-right">
          {isBool ? (
            <div className="flex items-center justify-end gap-1.5 h-full">
              {row.rawValue ? (
                <svg className="w-5 h-5 text-[#3ec099]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
              ) : (
                ...
                return (
                  <span
                    ...
                    className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold border transition-all ${
                      isPresent
                        ? "bg-[#3ec099]/10 border-[#3ec099]/30 text-[#3ec099]"
            ...
            <div className="flex justify-end gap-1">
              {(row.rawValue as string[]).map((v, i) => (
                <span key={i} className="bg-zinc-100 border border-zinc-200 px-2 py-0.5 rounded text-[10px] font-bold text-zinc-600 uppercase tracking-tight">
                  {v.replace(/_/g, ' ')}
                </span>
              ))}
            </div>
          ) : (
            <div className={`text-base truncate ${isEmpty(row) ? 'text-zinc-300 italic' : 'text-zinc-900 font-bold'}`}>
    ...
    <div className="rounded-xl border border-zinc-200 bg-white p-5 space-y-4 shadow-sm">
      <div className="flex items-center justify-between gap-2">
        ...
        {!isCollapsed && emptyCount > 0 && (
          ...
          >
            {showEmpty ? "Hide empty" : `+${emptyCount} empty indicators`}
          </button>
        )}
      </div>
      ...
      {visibleRows.length === 0 ? (
        <div className="py-10 text-center border-2 border-dashed border-zinc-100 rounded-2xl bg-zinc-50/50">
          <p className="text-sm text-zinc-400 font-bold">No data available.</p>
        </div>
      ) : (
  ...
  return (
    <div className={`${ui.card} p-6 space-y-5`}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h3 className="text-sm font-bold text-zinc-900 uppercase tracking-wider">Upload Assets</h3>
      ...
      >
        ...
        <div className="text-center">
          ...
          <p className="text-sm font-medium text-zinc-500 mt-1">
            or <span className="text-[#3ec099] underline underline-offset-4 decoration-2">click to browse</span>
          </p>
        </div>
      </div>
    ...
    return (
      <div className="p-6">
        ...
        <div className="mt-6 rounded-xl border border-rose-200 bg-rose-50 p-6 text-sm text-rose-700 shadow-sm">
          <strong className="block text-rose-900 mb-1">Access Error</strong>
      ...
      <div className="mx-auto max-w-7xl px-6 pt-2 pb-8 space-y-6">
        {isFetchingLinked && (
          <div className="flex justify-end">
            <span className="text-xs font-bold text-zinc-400 animate-pulse">Syncing cases…</span>
          </div>
        )}
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full text-sm">
                  <tbody className="divide-y divide-zinc-100">
                    ...
                    <tr>
                      <td className="py-2.5 pr-4 text-zinc-500 font-bold text-[10px] uppercase tracking-wider">Make</td>
                      <td className="py-2.5 font-bold text-zinc-700">{make}</td>
                    </tr>
                    <tr>
                      <td className="py-2.5 pr-4 text-zinc-500 font-bold text-[10px] uppercase tracking-wider">Model</td>
                      <td className="py-2.5 font-bold text-zinc-700">{model}</td>
                    </tr>
                    <tr>
                      <td className="py-2.5 pr-4 text-zinc-500 font-bold text-[10px] uppercase tracking-wider">Variant</td>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full text-sm">
                  <tbody className="divide-y divide-zinc-100">
                    ...
                    <tr>
                      <td className="py-2.5 pr-4 text-zinc-500 font-bold text-[10px] uppercase tracking-wider">Phone</td>
        ...
        <div className={`${ui.card} p-6 shadow-md`}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-zinc-900">Pictures & Assets ({caseAssets.length})</h2>
            <div className="flex items-center gap-3">
              {caseAssets.length > 0 && (
                <a
          ...
          {caseAssets.length === 0 ? (
            <div className="mt-8 py-12 text-center border-2 border-dashed border-zinc-100 rounded-2xl bg-zinc-50/50">
              <p className="text-sm font-bold text-zinc-400">No assets have been uploaded yet.</p>
            </div>
          ) : (
            <div className="mt-4">
              <AssetGallery
